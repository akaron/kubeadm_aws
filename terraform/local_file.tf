resource "local_file" "inventory" {
  ...
  content = templatefile("inventory.tmpl", {
    controlplane_ips = zipmap(range(length(data.aws_instances.controlplane.public_ips)), data.aws_instances.controlplane.public_ips)
    worker_ips = zipmap(range(length(data.aws_instances.worker.public_ips)), data.aws_instances.worker.public_ips)
